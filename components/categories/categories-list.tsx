...
// Fix: Type for API response and fallback for empty data
...
export function CategoriesList() {
  ...
  // State for sorting
  const [sortField, setSortField] = useState<"name" | "count" | "sortPosition">("name")
  ...
  // Defensive: ensure categoriesApiData is always an array/object with categories key
  let apiCategories: ApiCategory[] = [];
  if (
    categoriesApiData &&
    typeof categoriesApiData === "object" &&
    "categories" in categoriesApiData &&
    Array.isArray((categoriesApiData as any).categories)
  ) {
    apiCategories = (categoriesApiData as any).categories;
  }
  const categoriesData = apiCategories.map((cat) => ({
    id: cat.category_id,
    name: cat.name,
    count: 0, // No count in API, set to 0 or fetch separately if needed
    sortPosition: cat.sort_order ?? 0,
  ...
  const handleSort = (field: "name" | "count" | "sortPosition") => {
    if (sortField === field) {
